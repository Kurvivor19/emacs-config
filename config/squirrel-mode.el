...
(defcustom squirrel-indent-level 2
...
(defconst squirrel-defun-prompt-regexp
  (concat
   "\\s-*function\\s-+\\w+\\s-*\\|" ; function declaration
   "\\s-*constructor\\s-*\\|"       ; class constructor declaration
   "\\s-*class\\s-+\\w+\\s-*\\|"    ; class declaration of form class ClassName {...
   "\\w+\\s-*<-\\s-*class\\s-*\\|"  ; class declaration of form ClassName <- class {...
   "^"                              ; single-standing opening curled brace, probably not needed
   )
  "Regular expression for locating beginnings of function and class declarations")

(defconst squirrel-imenu-generic-expression
  (list
   '(nil "^\\s-*function\\s-+\\(\\w+\\)\\s-*" 1)
   '(nil "^\\s-*constructor\\s-*" 0)
   '(nil "\\s-*class\\s-+\\(\\w+\\)\\s-*" 1)
   '(nil "\\(\\w+\\)\\s-*<-\\s-*class\\s-*" 1))
  "Imenu support for function and class declarations"
  )
...
(defun squirrel-mode ()
  ...
  (setq-local defun-prompt-regexp squirrel-defun-prompt-regexp)
  (setq-local imenu-generic-expression squirrel-imenu-generic-expression)
